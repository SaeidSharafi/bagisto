<div>
    ...
    <div class="control-group {!! $errors->has('images.*') ? 'has-error' : '' !!}">
        ...
        <attribute-gallery></attribute-gallery>
...
@php
   $images = $product[$attribute->code] ? json_decode($product[$attribute->code]) : [];
@endphp


@push('scripts')

    <script type="text/x-template" id="attribute-gallery-template">
        <div>
            <div class="image-wrapper">
                <attribute-gallery-item
    ...
    <script type="text/x-template" id="attribute-gallery-item-template">
        <label class="image-item" v-bind:class="{ 'has-image': imageData.length > 0 }">
            <input type="hidden" :name="'{{$attribute->code}}[' + image.id + ']'" :value="image.path" v-if="! new_image"/>

            <input type="file" v-validate="'mimes:image/*'" accept="image/*" :name="'{{$attribute->code}}[]'"
                   ref="imageInput" :id="_uid" @change="addImageView($event)" multiple="multiple"/>
    ...
    <script>
        Vue.component('attribute-gallery', {

            template: '#attribute-gallery-template',
            ...
            computed: {
                finalInputName: function() {
                    return '{{$product[$attribute->code]}}[' + this.image.id + ']';
                }
            },
        ...
        Vue.component('attribute-gallery-item', {

            template: '#attribute-gallery-item-template',
