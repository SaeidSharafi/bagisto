<v-product-card {{ $attributes }} :product="product"> </v-product-card>

@pushOnce('scripts')
<script type="text/x-template" id="v-product-card-template">
    <!-- Grid Card -->
    <div
        class='grid gap-2.5 content-start w-full relative bg-[#F5F5F5]'
        v-if="mode != 'list'"
    >
        <div class="relative overflow-hidden group max-w-[291px] max-h-[300px] rounded-[4px]">
            ...
            >
                <x-shop::media.images.lazy
                    class="relative after:content-[' '] after:block after:pb-[calc(100%+9px)] bg-[#F5F5F5] group-hover:scale-105 transition-all duration-300 aspect-[30/25]"
                    src="/images/shop/product/small-product-placeholder.jpg"
                    ::key="product.id"
                    ::index="product.id"
                    width="291"
                    height="300"
        ...
        <div class="grid gap-2.5 content-start max-w-[291px]">
            <p class="text-base truncate" v-text="product.name"></p>
            <div
            class="flex gap-2.5 font-semibold text-lg"
            v-html="product.teacher_name"></div>
            <div
                class="flex gap-2.5 font-semibold text-lg"
